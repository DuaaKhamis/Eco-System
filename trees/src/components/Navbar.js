...
//                 key={item.name}
//                 href={item.href}
//                 className={`flex items-center hover:text-green-500 transition duration-300 ${pathname === item.href ? "border-b-2 border-green-500" : ""
//                   }`}
//               >
//                 {item.name}
//               </Link>
...
//                   key={item.name}
//                   href={item.href}
//                   className={`block py-2 px-4 hover:bg-green-50 ${pathname === item.href ? "bg-green-50 text-green-500" : ""
//                     }`}
      ...
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex space-x-1 items-center">
            ...
            <CartIcon className="w-6 h-6 text-white hover:text-green-200 transition duration-300" />
          </div>
          ...
          <div className="hidden md:flex items-center space-x-2">
            {isLoggedIn ? (
              <motion.button
                onClick={handleLogout}
                className="bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-4 rounded-full transition duration-300 flex items-center text-sm"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
              ...
              <>
                <Link href="/login">
                  <motion.button
                    className="bg-white text-green-500 hover:bg-green-100 font-medium py-2 px-4 rounded-full transition duration-300 flex items-center text-sm"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                  >
                ...
                <Link href="/signup">
                  <motion.button
                    className="bg-yellow-400 hover:bg-yellow-500 text-white font-medium py-2 px-4 rounded-full transition duration-300 text-sm"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                  >
                    Sign Up
          ...
          <div className="md:hidden">
            <button
              ...
              className="inline-flex items-center justify-center p-2 rounded-full text-white hover:bg-white hover:text-green-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white"
            >
              {isMenuOpen ? (
                <X className="w-6 h-6" />
              ) : (
              ...
              <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                {navItems.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className={`flex items-center px-3 py-2 rounded-full text-base font-medium hover:bg-white hover:text-green-500 transition duration-300 ${pathname === item.href ? "bg-white text-green-500" : ""
                      }`}
                    onClick={() => setIsMenuOpen(false)}
                  >
                ...
                {isLoggedIn && (
                  <Link
                    href="/profile"
                    className="flex items-center px-3 py-2 rounded-full text-base font-medium hover:bg-white hover:text-green-500 transition duration-300"
                    onClick={() => setIsMenuOpen(false)}
                  >
